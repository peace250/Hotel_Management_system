<?php
 ...
 if ($_SERVER['REQUEST_METHOD'] === "POST") {
     ...
     // automatically assign role('customer'):Default! 
     $role = 'customer';
     $password = htmlspecialchars(trim($_POST['password']));
     $confirmpassword = htmlspecialchars(trim($_POST['confirmpassword']));
     $hashedpassword = password_hash($password, PASSWORD_BCRYPT);
     $token = md5(uniqid(rand(), true));
     $insertQuery = "INSERT INTO users (email,password,role,phone,name,is_verified,token) VALUES (?,?,?,?,?,?,0,?)";
 ...
 <body class="d-flex justify-content-center">
     ...
     <form action="#" method="POST" class="col-sm-4 p-5 d-flex flex-column gap-2" style="height: fit-content;">
      ...
      border-bottom: 2px solid rgb(35, 35, 35); border-radius:0%">
         ...
         <div class="d-flex align-items-center">
             <input type="password" class="form-control" name="confirmpassword" id="passwordField" style="border: none;
      border-bottom: 2px solid rgb(35, 35, 35); border-radius:0%">
